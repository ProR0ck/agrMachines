<div class="container">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?= $route->map['category'] . $productInfo['id_category'] ?>"><?= $productInfo['category_name'] ?></a>
        </li>
    ...
    <div class="col-sm-8">
        <ul class="thumbnails">
            <li>
                <a class="thumbnail" href="<?= $link ?>/catalog/view/image/<?= $productMainPhoto['path'] ?>"
                   title="Apple Cinema 30&quot;"><img
                            src="<?= $link ?>/catalog/view/image/<?= $productMainPhoto['path'] ?>"
                            title="Apple Cinema 30&quot;" alt="Apple Cinema 30&quot;"></a>
            </li>
            <?php foreach ($productPhotos as $photo) { ?>
                <li class="image-additional">
                    <a class="thumbnail" href="<?= $link ?>/catalog/view/image/<?= $photo['path'] ?>"
                       title="Apple Cinema 30&quot;">
                        <img src="<?= $link ?>/catalog/view/image/<?= $photo['path'] ?>" title="Apple Cinema 30&quot;"
                             alt="Apple Cinema 30&quot;">
                    </a>
                </li>
            <?php } ?>
    ...
    <div class="col-sm-4">
        ...
        <ul class="list-unstyled">
            ...
            <li>
                <div style="display:inline-block;height: 15px; width: 15px; background-color: <?= $productInfo['color_code'] ?>"></div>
                Цвет - <?= $productInfo['color_name'] ?></li>
            <li>Доступность: <?= $productInfo['stock_status_value'] ?></li>
            <li><h2><?= $productInfo['price'] ?></h2></li>
            <li>
                <button type="button" id="button-cart" data-loading-text="Загрузка..."
                        class="btn btn-primary btn-lg btn-block"
                        onclick="window.location.href = '<?= $link ?>/add-to-basket/<?= $productInfo['id_vehicle'] ?>';">
                    В корзину
                </button>
            </li>
        </ul>
